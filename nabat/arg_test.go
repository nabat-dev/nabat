...
func TestRequiredArg_constructorDefaultDoesNotSatisfy(t *testing.T) {
	t.Parallel()

	io, _, _, _ := testIO()
	app := MustNew("test", WithIO(io))
	app.MustCommand("run",
		WithArg("env", "staging", WithRequired()),
		WithRun(func(c *Context) error {
			t.Fatal("handler should not run when required arg uses only a constructor default")
			return nil
		}),
	)
	err := Run(t, app, []string{"run"})
...
func TestRequiredArg_envSatisfies(t *testing.T) {
	t.Setenv("TEST_ENV", "production")

	io, _, _, _ := testIO()
	app := MustNew("test", WithIO(io))
	app.MustCommand("run",
		WithArg("env", "staging", WithRequired(), WithEnv("env")),
		WithRun(func(c *Context) error {
			got, err := c.BindAs[string]("env")
			require.NoError(t, err)
			assert.Equal(t, "production", got)
			assert.True(t, c.Explicit("env"))
			return nil
		}),
	)
	require.NoError(t, Run(t, app, []string{"run"}))
...
func TestRequiredArg_promptSatisfiesNotConstructorDefault(t *testing.T) {
	t.Parallel()
	...
	app := MustNew("test", WithIO(io))
	app.MustCommand("run",
		WithArg("name", "unused-default", WithRequired(), WithPrompt("Name", "", WithHint("alice"))),
		WithRun(func(c *Context) error {
			var err error
			got, err = c.BindAs[string]("name")
			require.NoError(t, err)
			explicit = c.Explicit("name")
			return nil
		}),
	)
	err := Run(t, app, []string{"run"})
	if err != nil {
		assert.NotContains(t, err.Error(), "required arg")
		return
	}
	assert.NotEqual(t, "unused-default", got)
	assert.True(t, explicit)
}

func TestRequiredArg_stderrPipeSkipsPromptAndDefault(t *testing.T) {
	t.Parallel()
	...
	app := MustNew("test", WithIO(io))
	app.MustCommand("run",
		WithArg("name", "staging", WithRequired(), WithPrompt("Name", "", WithHint("alice"))),
		WithRun(func(c *Context) error {
			t.Fatal("handler should not run when required arg cannot prompt")
			return nil
		}),
	)
	err := Run(t, app, []string{"run"})
